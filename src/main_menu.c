...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, 0x0000FFFF, SPECIES_TORCHIC);
...
{
    return CreatePicSprite2(SPECIES_TORCHIC, 8, 0, 1, a, b, 14, -1);
}
...
static void NewGameBirchSpeech_SetDefaultPlayerName(u8 nameId)
{
    ...
    if (gSaveBlock2Ptr->playerGender == MALE)
        name = gText_DefaultNameKai;
    else
        name = gText_DefaultNameMeili;
    for (i = 0; i < 7; i++)
