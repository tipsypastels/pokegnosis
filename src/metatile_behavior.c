...
static const u8 sTileBitAttributes[] =
    {
        ...
        [MB_LILY_PAD] = TILE_ATTRIBUTES(TRUE, TRUE, TRUE),
        [MB_BURROW] = TILE_ATTRIBUTES(TRUE, FALSE, TRUE),
...
{
    if (MetatileBehavior_IsSurfableWaterOrUnderwater(metatileBehavior) == FALSE && MetatileBehavior_IsEncounterTile(metatileBehavior) == TRUE && MetatileBehavior_IsBurrowWildEncounter(metatileBehavior) == FALSE)
        return TRUE;
    else
        return FALSE;
}
...
bool8 MetatileBehavior_IsLilyPadWildEncounter(u8 metatileBehavior)
{
    if (metatileBehavior == MB_LILY_PAD)
        return TRUE;
    else
        return FALSE;
}

bool8 MetatileBehavior_IsBurrowWildEncounter(u8 metatileBehavior)
{
    if (metatileBehavior == MB_BURROW)
        return TRUE;
    else
        return FALSE;
}
